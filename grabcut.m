...
pix_U = alpha==1;
T_U = im_1d(pix_U, :); % 选择的图片
pix_B = ~pix_U;
T_B = im_1d(pix_B, :); % 没有选择的图片

%%% Initialize GMM
no_gauss = 5; % 5 Gaussians in each GMM % k-means分类后的类数
% Background % 系统自带k-means
k_B = kmeans(T_B, no_gauss, 'Distance', 'cityblock', 'Replicates', 5);
gmm_B = fit_gmm(T_B, k_B); % Fit a GMM(高斯混合模型) with K Gaussians where K=numel(unique(labels))
% Foreground
...
isConverged = 0;
E_prev = +Inf; % Engry function
iter = 0;
while ~isConverged
    
    %------- 1. Assign GMM components to pixels % 用GMM进行第一次分类
