...
beta = compute_beta(im_sub); % 计算像素点与右像素点和下像素点的2-范数
...
for y = 1:im_h
    for x = 1:im_w
        ...
        if x < im_w % Has a right neighbor
            ...
            pairwise(idx, 3) = compute_V(color, 0, color_r, 0, gamma, beta); % compute the smoothness term
...
for y = 1:im_h
    for x = 1:im_w
        % Current node
        color = get_rgb_double(im_sub, x, y); % 取double
        
        % Right neighbor
        if x < im_w % Has a right neighbor
            color_r = get_rgb_double(im_sub, x+1, y); % 取double
            beta_sum = beta_sum+norm(color-color_r)^2; % 取2-范数
            cnt = cnt+1;
        end
        % Down neighbor
        if y < im_h % Has a down neighbor
            color_d = get_rgb_double(im_sub, x, y+1); % 取double
            beta_sum = beta_sum+norm(color-color_d)^2; % 取2-范数
            cnt = cnt+1; 
        end
    end
end
